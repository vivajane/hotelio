import React from "react";
import Link from "next/link";

const Footer = () => {
  return (
    <div className="bg-[#9a9591] pb-12 pt-4 px-4 sm:px-8 md:px-16 lg:px-24">
    <div className="md:flex justify-between  gap-8 py-10">
      <div>
       <Link href="/">
        <h1 className= "text-2xl font-roboto font-bold">
          Hotelio
        </h1>
       </Link>
        <p className="py-2">9 Obafemi Road, 6th Floor, Lekki Phase 1, Lagos</p>
        <p>Lagos State</p>
      </div>
      <div>
        <h1 className="text-md py-2 font-bold">Discover</h1>
        <ul className="py-2 space-y-2">
          <li>
            <Link href="/about">About</Link>
          </li>
          <li>
            <Link href="/activities">Facilities</Link>
          </li>
          <li>
            <Link href="/faq">FAQ</Link>
          </li>
        </ul>
      </div>
      <div>
        <h1 className="text-md py-2 font-bold">Support</h1>
        <ul className="py-2 space-y-2">
          <li>
            <a href="https://wa.me/+2348116070090">Whatsapp</a>
          </li>
          <li>
            <a href="mailto:hotelio@gmail.com"></a>
          </li>Email
        </ul>
      </div>
      <div>
        <h1 className="text-md py-2 font-bold">Legal</h1>
        <ul className="py-2 space-y-2">
          <li>
            <Link href="/policies">Privacy Policies</Link>
          </li>
          <li>
            <Link href="/terms">Terms & Conditions</Link>
          </li>
        </ul>
      </div>
    </div>
    <div className="md:text-center">
        <p className="md:text-sm text-xs">Copyright © 2025 Hotelio All Rights Reserved. v1.30.2</p>
    </div>
    </div>
  );
};

export default Footer;
